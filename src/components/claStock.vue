...
<script>
  ...
  export default {
    data () {
      return {
        urlApi: [
          // sk_07 200.1.3.89:1209 shda.91onix.com
          'http://shda.91onix.com/App/AppStock/ContrastFacetByVariety'
        ],
        yes: true,
        isShow: false,
        myChart: Object,
    ...
    methods: {
      goEcharts1 () {
        this.isShow = false;
        this.yes = true;
        this.$set('duty','库存码洋占比')
        this.$set('tabName','库存码洋') 
      ...
      goEcharts2 () {
        this.isShow = true;
        this.yes = false;
        this.$set('duty','库存品种数占比')
        this.$set('tabName','库存品种数') 
      ...
      whichEcharts (arr,condition,op1,op2) {
        ...
        if(valNum == arr.length){
          ...
          this.$set('supplierName',arr[this.num].Classify)
      ...
      clickEcharts (arr,params,_this,op1,op2) {
        ...
        for(let i=0; i<arr.length; i++){
          ...
          if(arr[i].Classify == params.name){
            ...
            _this.$set('supplierName',arr[i].Classify)
